 ...
 * @version 1.0
 */
package org.xottys.lifecycle.activitylifecycle.ActivityTaskMode;
...
public class StandardActivity extends Activity {
    ...
    public void onResume()
    {
        ...
        for (ActivityManager.RunningTaskInfo runningTaskInfo : runningTasks) {
            tv.append("-----"+(runningTaskInfo.topActivity.getShortClassName()).substring(18)+"-----\n");
            Log.v(TAG, runningTaskInfo.id+"---"+runningTaskInfo.numActivities+"---"+runningTaskInfo.baseActivity.getShortClassName()+"---"+(runningTaskInfo.topActivity.getShortClassName()).substring(18));
        }

        //输出Activity栈中间的内容
        String str= MyApplication.getInstance().createtActivityStackInfo();
        tv.setText(str);
    }
}
